...
sed -i "s/\.//g" data/pep.fa
python /usr/bin/RemoveStarFromFasta.py -i data/pep.fa -o data/proteins.fa
...
awk '{print $1}' results/2-interproscan.bed | uniq > results/2-interproscan.id.txt
perl /usr/bin/get_fa_by_id.pl results/2-interproscan.id.txt data/proteins.fa results/3-extracted.fasta
...
bash /usr/bin/interproscan.sh -f TSV -app SUPERFAMILY -i results/3-extracted.fasta -b results/6-lrr_domain
bash code/IPS2fpGs.sh -f -c code/conf1.tsv -d code/conf2.tsv -o results/7.1-full_or_partial.tsv results/1-interproscan.tsv
bash code/IPS2fpGs.sh -f -c code/conf1.tsv -d code/conf2.tsv -o results/7.2-full_or_partial.tsv results/6-lrr_domain.tsv
...
/home/software/genblast/genblastg -q results/9-full_length.fasta -t data/genome.fa -gff -cdna -pro -o results/10-genblastG_out
perl /usr/bin/agat_sp_filter_gene_by_length.pl --gff results/10-genblastG_out_1.1c_2.3_s1_0_16_1.gff --size 20000 --test "<" -o results/11-filtered.length.gff
...
/usr/bin/python3 /home/publicdata/eggnog-mapper/eggnog-mapper-v2.1.8/emapper.py -i data/proteins.fa --output results/15.emapper.out -m diamond --cpu 20
